...
import {
  selectCurrentUser,
  useCurrentToken
} from "../../redux/feathers/auth/authSlice";
  ...
  const checkStockAvailability = async (cart: TProduct[]) => {
    try {
      const outOfStockItems = cart.filter(
        (item) => item.quantity > (item.inStock ?? 0)
      );
      if (outOfStockItems.length > 0) {
        const outOfStockDetails = outOfStockItems
          .map(
            (item) =>
              `${item.model} (Qty: ${item.quantity}, Available: ${item.inStock})`
          )
          .join(", ");
        message.error(
          `Out of stock: ${outOfStockDetails}. Please update your cart.`
        );
        return false;
      }
      return true;
      ...
      <div className="flex-grow">
        {isLoading ? (
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="mt-6 text-center sm:text-right">
              ...
              <div className="flex flex-col sm:flex-row justify-center sm:justify-between items-center mt-4 gap-4">
                <Link to="/all-cars" className="w-full sm:w-auto">
          ...
          <div className="flex flex-col justify-center items-center h-full">
            ...
            <Link to="/all-cars" className="mt-4">
