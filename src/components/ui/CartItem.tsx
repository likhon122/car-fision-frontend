...
const CartItem: React.FC<CartItemProps> = ({
  ...
  const handleIncrease = () => {
    if (item.inStock !== undefined && item.quantity < item.inStock) {
      ...
      message.success("Product Increased successfully!");
    }
  };
  ...
  const handleDecrease = () => {
    if (item.quantity > 1) {
      onUpdateQuantity(item._id, item.quantity - 1);
      message.success("Product Decrease successfully!");
    }
  };
    ...
    <div className="flex flex-col md:flex-row items-center justify-between bg-white p-4 shadow rounded-lg space-y-4 md:space-y-0 md:space-x-4">
      <div className="flex items-center w-full justify-between md:!justify-start gap-[30px] md:gap-[20px]">
        ...
        <div className="text-center md:text-left flex-1">
          <h2 className="text-lg md:text-[20px] font-semibold">{item.model}</h2>
          <p className="text-sm text-gray-500">{item.brand}</p>
          <p className="text-sm md:text-[20px] text-gray-500">
      ...
      <div className="flex flex-col md:flex-row items-center md:space-x-4 space-y-4 md:space-y-0">
        <div className="flex items-center gap-[10px]">
          ...
          <Button
            ...
            disabled={
              item.inStock === undefined || item.quantity >= item.inStock
            }
          >
            <span className="text-[24px] mt-[-2px]">+</span>
          </Button>
        </div>
        <Button type="primary" danger onClick={() => onRemove(item._id)}>
          Remove
        </Button>
      </div>
    </div>
